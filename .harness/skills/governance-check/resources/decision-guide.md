...
flowchart TD
    ...
    B -- "Sí" --> C["Rule document<br/>.harness/rules/<br/><br/>Ejemplos: architecture.md<br/>uv-dependency-management.md<br/>testing.md<br/>git-conventions.md<br/>definition-of-done.md"]
    ...
    N -- "Sí" --> O["Skill<br/>.harness/skills/<nombre>/SKILL.md<br/><br/>Integra aquí el procedimiento ordenado<br/>cuando forma parte de esta capacidad"]

    N -- "No" --> P{"¿El valor principal está en una<br/>secuencia breve, fija y ordenada<br/>invocada explícitamente?"}

    P -- "Sí" --> Q["Standalone Workflow<br/>.harness/workflows/<nombre>.md<br/><br/>Proceso breve, autocontenido<br/>y ejecutable bajo demanda"]
...
Todo artefacto agéntico en este repositorio debe ubicarse en su carpeta canónica bajo `.harness/` y seguir la convención de nomenclatura `kebab-case`:

| Tipo de Artefacto | Ruta Canónica de Destino | Convención de Fichero | Estructura Mínima Obligatoria |
| --- | --- | --- | --- |
| **Rule** | `.harness/rules/<nombre-en-kebab-case>.md` | `<nombre>.md` | Frontmatter con `trigger:` (`always_on`, `glob`, `model_decision`, `manual`) + Texto normativo con MUST/MUST NOT. |
| **Workflow** | `.harness/workflows/<nombre-en-kebab-case>.md` | `<nombre>.md` | Frontmatter con `description:` + Pasos numerados secuenciales con resultado esperado por paso. |
| **Skill** | `.harness/skills/<nombre-en-kebab-case>/SKILL.md` | Directorio `<nombre>/` y fichero `SKILL.md` | Frontmatter con `name:` y `description:` (con criterios de activación) + Cuándo usar/no usar, procedimiento y verificación. Opcional: `scripts/`, `resources/`. |
| **Hook** | `.harness/hooks/<nombre-en-kebab-case>.md` | `<nombre>.md` | Definición conceptual: evento detonante (trigger), condiciones, validación y acción de bloqueo en fallo. |
| **Template** | `.harness/templates/<nombre-en-kebab-case>.md` | `<nombre>.md` | Estructura con marcadores `<campo>` e instrucciones de uso. |
| **Agent Profile** | `.harness/agents/<nombre-en-kebab-case>.md` | `<nombre>.md` | Rol, responsabilidades, herramientas permitidas/excluidas y contexto de activación. |
...
> **Nota Crítica sobre Adaptadores Específicos:** Los ficheros en `.github/`, `.claude/`, `.cursor/` o `.gemini/` son únicamente **puentes mínimos**. NUNCA crees o dupliques reglas, skills o workflows directamente en esas carpetas; la fuente canónica siempre es `.harness/`.

---

## 3. Criterios de Selección Detallados por Tipo de Artefacto

### A. Rules (`.harness/rules/`)
...
### F. Agent Profiles (`.harness/agents/`)
...
- **`workspace` (`.harness/...`)**:
  ...
  - **Ubicación:** Dentro de la raíz del proyecto en `.harness/`.
- **`global` (`~/.harness/...`)**:
  ...
  - **Ubicación:** En el home del usuario (ej. `~/.harness/` o configuración de IDE).
...
| --- | --- | --- | --- |
| *"Quiero que nunca se hagan commits directamente a main y que los mensajes sigan Conventional Commits"* | **Rule** | `.harness/rules/git-workflow.md` | Es una restricción normativa permanente (`trigger: always_on`). |
| *"Crea un artefacto para validar que todos los endpoints de FastAPI tengan response_model explícito"* | **Rule** | `.harness/rules/fastapi-endpoints.md` | Restricción arquitectónica de código (`trigger: glob: app/routers/**/*.py`). |
| *"Crea un artefacto para estimar historias de usuario usando el modelo CAG y generar el JSON final"* | **Skill** | `.harness/skills/cag-estimator/SKILL.md` | Capacidad especializada con instrucciones, ejemplos y posiblemente scripts auxiliares. |
| *"Crea un artefacto que defina la secuencia para preparar una release: tests, subir versión, changelog y tag"* | **Workflow** | `.harness/workflows/release-process.md` | Secuencia procedimental ordenada y ejecutable bajo demanda. |
| *"Crea un artefacto con la plantilla estándar para redactar ADRs (Architecture Decision Records)"* | **Template** | `.harness/templates/adr-template.md` | Estructura esqueleto reutilizable con campos a rellenar. |
| *"Crea un artefacto para un rol de auditor de seguridad que solo revise vulnerabilidades y no edite código"* | **Agent Profile** | `.harness/agents/security-auditor.md` | Definición de rol/persona con restricción de herramientas y foco específico. |
| *"Crea un artefacto que verifique que no se dejen contraseñas ni API keys antes de confirmar una tarea"* | **Hook** | `.harness/hooks/pre-commit-secrets-check.md` | Compuerta de validación en el ciclo de vida del agente. |
...
Cada vez que se cree un nuevo artefacto en `.harness/`, se debe registrar en `.harness/manifest.yaml` bajo la sección correspondiente de `inventory`:

```yaml
inventory:
  rules:
    - id: mi-regla
      path: .harness/rules/mi-regla.md
      description: Breve descripción de la restricción.
      applies_to: [coding, git]
  skills:
    - id: mi-skill
      path: .harness/skills/mi-skill/SKILL.md
