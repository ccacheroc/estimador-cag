...
description: Validates whether content belongs in a Rule, Skill, Hook, Template, or Agent, and executes the governance verification procedure and checklists before merging agentic changes. Use when reviewing, creating, or modifying agentic configuration files, AGENTS.md, or .harness/ artifacts.
author: ccachero
version: 1.0.0
---
...
### Use this skill when:
- Reviewing, creating, or modifying files in `AGENTS.md`, `/.harness/`, or tool bridges (`.claude/`, `.github/`, `.cursor/`, `.gemini/`).
...
- Performing ordinary code changes, bug fixes, or feature development that do not modify agentic architecture or `.harness/` configurations.
...
Para clasificar cualquier nuevo requerimiento, determinar el tipo exacto de artefacto a crear (**Rule**, **Workflow**, **Skill**, **Hook**, **Template**, **Agent Profile**, o **Canonical Documentation**), resolver la ruta canónica en `.harness/` o `docs/`, la convención de nombres y el modo de activación:
...
1. Identify all files added, modified, or deleted in `AGENTS.md`, `/.harness/`, or tool adapters, or the requirement **X** requested by the user.
...
For each artifact:
- Confirm whether it belongs to `global` (`~/.harness/`) or `workspace` (`.harness/`).
- If a **Rule**: ensure the least-powerful activation mode is configured (`manual`, `always_on`, `glob`, `model_decision`).
- Verify registration in `.harness/manifest.yaml` under `inventory:`.
